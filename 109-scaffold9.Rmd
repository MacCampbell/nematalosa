...
```{sh,eval=FALSE}
bcftools view -r scaffold_9:14123901-35865842  data/gl_vcf.vcf.gz > outputs/109/inv.vcf

bcftools +fill-tags outputs/109/inv.vcf  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.10' | bcftools view -i 'INFO/MAF > 0.01'   > outputs/109/filt.vcf

bcftools +fill-tags outputs/109/inv.vcf  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.10' | bcftools view -i 'INFO/MAF > 0.01'  |  bcftools +prune -m 0.20 -w 50kb --nsites-per-win 1 --nsites-per-win-mode rand> outputs/109/pruned.vcf
...
Macumba_NEMacumba.1	0.423676
vlaminghi_NVMurray.2	0.426791
Machettie_NEMachettie.1	0.570093
Peake_NEPeake.2	0.669782
Cunnamulla_NE1423.10	0.672897
PU1425_NE1425.2	0.676012
PU14115_NE14115.2	0.679128
Victoria_NEHumbert.1	0.694704
Ashburton_NEAshburton.3	0.713396
PU09110_NE09110.2	0.713396
...
```{r}
dat<-read.dna(file="outputs/109/phylo/filt.min4.phy.varsites.phy")
write.nexus.data(dat, file="outputs/109/phylo/filt.nex")
```

```{r}
net<-read.nexus.networx("outputs/109/phylo/filt-network")
m<-meta
```
...
ggsave("outputs/109/inv-network.pdf", width=12, height=12)
...
```{r}
tree<-read.tree("outputs/109/phylo/filt.min4.phy.varsites.phy.contree")
...
ggsave("outputs/109/inv-phylo-9.pdf", width=8, height=24)
```
...
ggsave("outputs/109/inv-phylo-homs-9.pdf", width=8, height=24)
```
...
```{r}
h<-read_vcf("outputs/109/filt.vcf")

h2<-calc_hs(h)
h3<-get.snpR.stats(h2, stats="hs")

dh<-left_join(df09, h3$sample, by=c("Label"="sampID")) %>% left_join(meta) 
...
ggplot(dh) +
  ...
  ggtitle("Chromosome 9 Inversion Heterozygosities") +
  theme(plot.title=element_text(hjust=0.5)) #+
#  facet_wrap(.~Taxon)

ggsave("outputs/109/inv-09-heterozygosity.pdf")
```